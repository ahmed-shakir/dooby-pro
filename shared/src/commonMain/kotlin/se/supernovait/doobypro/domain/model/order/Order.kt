 ...
 * @property deliveryDatetime The scheduled delivery date and time.
...
data class Order(
    ...
    /**
     * Determines the next logical status in the workflow.
     *
     ...
     * Orders can be cancelled as long as they are not in a terminal state.
     */
    fun canCancel(): Boolean = !status.isTerminal()

    /**
     * Checks if the order can be deleted.
     * Only new orders that haven't been processed yet can be deleted.
     */
